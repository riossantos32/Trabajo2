...
const TarjetaCategoria = ({
  ...
  // Efecto para detener el spinner cuando hay datos [cite: 8]
  useEffect(() => {
    setCargando(!(categorias && categorias.length > 0));
  }, [categorias]);

  // Cerrar opciones al presionar Escape [cite: 8]
  ...
  return (
    <>
      {cargando ? (
        // Vista de carga [cite: 9]
        <div className="text-center my-5">
          <h5>Cargando categorías...</h5>
          <Spinner animation="border" variant="success" role="status" />
        </div>
      ) : (
        // Listado de tarjetas [cite: 9]
        <div>
              ...
              >
                <Card.Body 
                  className={`p-2 tarjeta-categoria-cuerpo ${
                    tarjetaActiva ? "tarjeta-categoria-cuerpo-active" : "tarjeta-categoria-cuerpo-inactive"
                  }`}
                >
                ...
                {/* Capa de botones que aparece al seleccionar la tarjeta [cite: 9] */}
                {tarjetaActiva && (
                  <div 
